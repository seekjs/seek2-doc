...
define("sys.dialog",function (req, exp) {
	"use strict";

exp.getTemplate = function($){var buf = [];
buf.push('﻿');
	var ops=this;
buf.push('<div class="sk-dialog" style="width:');
buf.push(ops.width);
buf.push('px; height:');
buf.push(ops.height);
buf.push('px;">	');
if(!ops.nobar){
buf.push('	<div class="sk-dialog-bar">		<p class="sk-dialog-title">');
buf.push(ops.title);
buf.push('</p>		<p class="sk-dialog-close" data-event="pickButton:close">×</p>	</div>	');
}
buf.push('	<div class="sk-dialog-content" style="height:');
buf.push(ops.btn?ops.height-110:ops.height-50);
buf.push('px;">		');
if(ops.url){
buf.push('		<iframe class="sk-dialog-content-iframe" src="');
buf.push(ops.url);
buf.push('"></iframe>		');
}else{
buf.push('		<div>');
buf.push(ops.content);
buf.push('</div>		');
}
buf.push('	</div>	');
if(ops.btn){
buf.push('	<div class="sk-dialog-buttons">		');
var sn,src;for(var i in ops.btn){
var item=ops.btn[i];
sn=i*1+1;
src={length:ops.btn.length, first:i==0, last:i==ops.btn.length-1};
buf.push('		<p class="sk-dialog-btn-');
buf.push(i);
buf.push('" data-event="pickButton:');
buf.push(i);
buf.push('">');
buf.push(item);
buf.push('</p>		');
